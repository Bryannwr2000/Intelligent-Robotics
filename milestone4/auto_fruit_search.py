...
def drive_to_point(waypoint, robot_pose):
    ...
    # Compute the angular displacement required by the robot to face the waypoint 
    angular_displacement = 0.5 * baseline * turning_angle
    
    # turn towards the waypoint 
    if (turning_angle < 0): # rotating clockwise
        turn_time = - angular_displacement / (wheel_vel * scale)
        print("Turning for {:.2f} seconds".format(turn_time))
        ppi.set_velocity([0, 1], turning_tick=-wheel_vel, time=turn_time)
    else: # rotating anticlockwise
        turn_time = angular_displacement / (wheel_vel * scale) # replace with your calculation
        print("Turning for {:.2f} seconds".format(turn_time)) 
        ppi.set_velocity([0, 1], turning_tick=wheel_vel, time=turn_time)
    
    # after turning, drive straight to the waypoint
    drive_time = distance_between_waypoint_and_robot / (wheel_vel * scale) # replace with your calculation
