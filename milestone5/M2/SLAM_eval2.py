...
def solve_trans_rot(points1, points2):
    # point_1 = robot_real_pose
    # point_2 = robot_slam_pose
...
def slam_eval():
    ...
    robot_real_pose = np.hstack([gt_robot_x, gt_robot_y, gt_robot_theta])
    robot_pose = np.hstack([robot_x, robot_y, robot_theta])
    theta, x = solve_trans_rot(robot_real_pose, robot_pose) #custom
    ...
    rmse_cus_aligned = compute_rmse(us_vec_aligned, gt_vec)
    
    
    
    save_txt(us_vec_aligned)
    ...
    plt.legend(['Real','Predicted', 'Pred_aligned(Trans)'])
    plt.grid()
    plt.show()
